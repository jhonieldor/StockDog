...
angular.module('stockDogApp')
  .service('WatchlistService', function WatchlistService($filter) {
    // Augment Stocks with additional helper functions
    var filteredWatchlists = [];
    var StockModel = {
      save: function () {
    ...
    this.addFilter = function(watchlist){
      if(filteredWatchlists.length === Model.watchlists.length)
        filteredWatchlists = [];

      if($filter('filter')(filteredWatchlists, {'name':watchlist.name})[0]==null){
